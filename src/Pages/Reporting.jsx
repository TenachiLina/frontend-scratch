...
// Module-level cache (persists while tab is open)
...
export default function Reporting() {
  ...
  // FETCH EMPLOYEES (with cache and localStorage update)
  const fetchEmployees = async (force = false) => {
    if (!force && employeesCache) {
      setEmployees(employeesCache);
      if (employeesCache.length) setSelectedEmployeeId(employeesCache[0].emp_id || employeesCache[0].id);
      return;
    }
    ...
    try {
      const list = await employeesApi.getEmployees();
      setEmployees(list);
      employeesCache = list; // update module-level cache
      if (list.length) setSelectedEmployeeId(list[0].emp_id || list[0].id);
    } catch (err) {
      console.error("Failed to load employees:", err);
      setEmployees([]);
  ...
  // Listen to localStorage updates from Employee Management page
  useEffect(() => {
